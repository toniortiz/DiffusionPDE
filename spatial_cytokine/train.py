...
@click.option(
    "--cond", help="Train class-conditional model", metavar="BOOL", type=bool, default=False, show_default=True
)
@click.option(
    "--arch",
...
@click.option(
    "--batch", help="Total batch size", metavar="INT", type=click.IntRange(min=1), default=512, show_default=True
)
...
@click.option(
    "--ema", help="EMA half-life", metavar="MIMG", type=click.FloatRange(min=0), default=0.5, show_default=True
)
@click.option(
    "--dropout",
...
# Performance-related.
@click.option(
    "--fp16", help="Enable mixed-precision training", metavar="BOOL", type=bool, default=False, show_default=True
)
@click.option(
    "--ls",
    help="Loss scaling",
...
def main(**kwargs):
    ...
    if opts.arch == "ddpmpp":
        c.network_kwargs.update(
            model_type="SongUNet", embedding_type="positional", encoder_type="standard", decoder_type="standard"
        )
        c.network_kwargs.update(
        ...
        c.network_kwargs.update(
            model_type="SongUNet", embedding_type="fourier", encoder_type="residual", decoder_type="standard"
        )
        c.network_kwargs.update(
            channel_mult_noise=2, resample_filter=[1, 3, 3, 1], model_channels=128, channel_mult=[2, 2, 2]
        )
    else:
