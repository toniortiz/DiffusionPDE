...
def training_loop(
    ...
    interface_kwargs = dict(
        data_resolution=dataset_obj.resolution, img_channels=dataset_obj.num_channels, label_dim=dataset_obj.label_dim
    )
    ...
    if dist.get_rank() == 0:
        with torch.no_grad():
            data = torch.zeros(
                [batch_gpu, net.img_channels, net.data_resolution, net.data_resolution, net.data_resolution],
                device=device,
            )
    ...
    augment_pipe = (
        dnnlib.util.construct_class_by_name(**augment_kwargs) if augment_kwargs is not None else None
